...
def load_info():
    messagebox.showinfo(
        "info", "This is PoorBoy ToDo V.1.0 \n created by Michael Sammons \n With Python")
...
label_title = tkinter.Label(root, text="PoorBoy ToDo", bg= "Gray")
label_title.grid(row=0, column=0)

label_dsply = tkinter.Label(root, text="", bg="SlateGray3")
label_dsply.grid(row=0, column=1)

label_dsp_count = tkinter.Label(root, text="", bg="SlateGray3")
...
text_add_bttn = tkinter.Button(
    root, text="Add Todo", bg="mint cream", fg="Black", width=15, command= add_task)
text_add_bttn.grid(row=1, column=0)

delone_bttn = tkinter.Button(
    root, text="Done Task", bg="mint cream", width=15, command=delete_one)
delone_bttn.grid(row=2, column=0)

delall_bttn = tkinter.Button(
    root, text="Delete All", bg="mint cream", width=15, command= delete_all)
delall_bttn.grid(row=3, column=0)

sort_asc = tkinter.Button(root, text ="Sort (ASC)",
                            bg="mint cream", width=15, command=sort_asc)
sort_asc.grid(row=4, column=0)

sort_dsc = tkinter.Button(root, text="Sort (DSC)",bg="mint cream", width=15, command=sort_dsc)
sort_dsc.grid(row=5, column=0)

random_bttn = tkinter.Button(root, text="Random Task", bg="mint cream", width=15, command=random_task)
random_bttn.grid(row=6, column=0)

number_task = tkinter.Button(root, text="Number of Task", bg="mint cream", width=15, command=number_task)
number_task.grid(row=7, column=0)

exit_bttn = tkinter.Button(root, text="Exit App", bg="mint cream", width=15, command=exit_app)
exit_bttn.grid(row=8, column=0)

save_button = tkinter.Button(root, text="Save Todo", bg="mint cream", width=15, command=save_act)
save_button.grid(row=10, column=1)

load_button = tkinter.Button(root, text="Load Last Todo", bg="mint cream", width=15, command=load_act)
load_button.grid(row=10, column=0)

info_button = tkinter.Button(root, text="Info", bg="mint cream", width=15, command=load_info)
